...
export default function LineoutSection({ stats, events, addEvent, deleteEvent, canEdit }) {
    const lo = stats?.lineouts || { ours: 0, won: 0, lost: 0, stolen: 0 };
    const recent = (events || []).filter(e => TYPES.some(t => t.type === e.type)).slice(-5).reverse();
